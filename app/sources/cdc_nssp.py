...
This is the numeric SPINE of the dashboard: weekly, per-virus, and it includes
Minnesota. CDC publishes this percentage at STATE level only (the sibling
dataset cdc_ed_trends.py provides the local, sub-state *direction*).
...
The numbers mean: of all ER visits in the state that week, what share were for
    ...
    for virus, vrows in by_virus.items():
        ...
        out.append(
            Series(
                ...
                signal_label="% of ER visits (statewide)",
                unit="%",
                points=points,
                provenance=Provenance(
                    ...
                    fetched_at=now_iso(),
                    is_sample=not LIVE,
                    notes="Based on ~80% of US emergency departments. Published statewide only.",
                ),
                current_value=current.value if current else None,
                current_date=current.date if current else None,
                trend=trend,
                trend_label=trend_label,
                description="Share of statewide ER visits that were for this illness.",
            )
        )
    return out
